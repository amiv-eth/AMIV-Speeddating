...
// created by Stephan J. Müller, heap.ch
// https://github.com/stepmuel/SJMForm/
...
class SJMForm {
	...
	private function produce_radio($e) {
		...
		foreach ($e['childNodes'] as $c) {
			...
			$out .= "\t<label for=\"$e[name]_$c[name]\">".htmlentities($c['args'][0])."</label>\n";
			$out .= "</dd>\n";
		}
		return $out;
	}
	private function produce_checkbox($e) {
		...
		foreach ($e['childNodes'] as $c) {
			...
			$checked = ($c['args'][1]) ? ' checked' : '';
			$out .= "<dd>\n";
			$out .= "\t<input type=\"checkbox\" name=\"$e[name]_$c[name]\" value=\"1\" id=\"$e[name]_$c[name]\"$checked>\n";
			$out .= "\t<label for=\"$e[name]_$c[name]\">".htmlentities($c['args'][0])."</label>\n";
			$out .= "</dd>\n";
		}
		return $out;
