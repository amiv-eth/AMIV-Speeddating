...
def index():
    try:
        ...
        if event != None:
            eventid = event.ID
            timeslots = TimeSlots.query.filter(TimeSlots.EventID==eventid).all()
    ...
    if request.method == 'GET':
        ...
        try:
            ...
            if slots != None:
                ...
                for slot in slots:
                    w = Participants.query.order_by((Participants.CreationTimestamp)).filter(Participants.EventID==eid, Participants.AvailableSlot==slot.ID, Participants.Gender == '1').all()
                    m = Participants.query.order_by((Participants.CreationTimestamp)).filter(Participants.EventID==eid, Participants.AvailableSlot==slot.ID, Participants.Gender == '0').all()
                    women.append(w)
                    men.append(m)
        except Exception as e:
            print(e)
...
def timeslot_view(timeslot_id):
    if request.method == 'GET':
        ...
        try:
            ...
            women = Participants.query.order_by((Participants.CreationTimestamp)).filter(Participants.AvailableSlot==slotid, Participants.Gender == '1').all()
            men = Participants.query.order_by((Participants.CreationTimestamp)).filter(Participants.AvailableSlot==slotid, Participants.Gender == '0').all()
...
def timeslot_view_ongoing(timeslot_id):
    ...
    try:
        ...
        women = Participants.query.order_by((Participants.CreationTimestamp)).filter(Participants.AvailableSlot==timeslot_id, Participants.Gender == '1', Participants.Present == '1').all()
        men = Participants.query.order_by((Participants.CreationTimestamp)).filter(Participants.AvailableSlot==timeslot_id, Participants.Gender == '0', Participants.Present == '1').all()
        event = Events.query.filter(Events.ID==slot.EventID).first()
    except Exception as e:
        print(e)
        return render_template('error.html')
...
def register_participant(event_id, participant_id, register):
    try:
        registered = event_change_register_status(db.session, participant_id, register)
    except Exception as e:
        print(e)
        return render_template('error.html')
    if registered:
...
def signup():
            ...
            if timeslots != None:
                ...
                for s in timeslots:
                    if s.SpecialSlot == 1:
                        ...
                        women = Participants.query.filter(Participants.AvailableSlot==s.ID, Participants.Confirmed=='1', Participants.Gender == '1').count()
                        men = Participants.query.filter(Participants.AvailableSlot==s.ID, Participants.Confirmed=='1', Participants.Gender == '0').count()
                        stri = '&nbsp &nbsp &nbsp'
    ...
    if request.method == 'POST' and form.validate():
        try:
            ...
            count = Participants.query.filter(Participants.EMail==email, Participants.EventID==eventid).count()
            chosen_timeslot = TimeSlots.query.filter(TimeSlots.ID==int(slots)).first()
